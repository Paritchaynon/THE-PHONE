...
exports.ARCHETYPE_CRITERIA = void 0;
exports.calculateArchetype = calculateArchetype;
exports.calculateCoupleDynamic = calculateCoupleDynamic;
exports.ARCHETYPE_CRITERIA = {
    THE_GUARDIAN: {
...
function calculateArchetype(traits) {
    let bestArchetype = 'THE_ANCHOR';
    let bestScore = -Infinity;
    for (const [archId, criteria] of Object.entries(exports.ARCHETYPE_CRITERIA)) {
        let score = 0;
        if (criteria.weights) {
            for (const [t, w] of Object.entries(criteria.weights)) {
                score += (traits[t] ?? 50) * w;
            }
        }
        if (criteria.negativeWeights) {
            for (const [t, w] of Object.entries(criteria.negativeWeights)) {
                score -= (traits[t] ?? 50) * w;
            }
        }
        if (score > bestScore) {
...
function calculateCoupleDynamic(archetypeA, archetypeB, relationship) {
    const set = new Set([archetypeA, archetypeB]);
    // Pair-based rules
    if (archetypeA === archetypeB && archetypeA === 'THE_MIRROR')
        return 'THE_MIRROR_AND_THE_MIRROR';
    if (archetypeA === archetypeB && archetypeA === 'THE_HEALER')
        return 'THE_HEALERS';
    if (set.has('THE_FIRE') && (set.has('THE_ANCHOR') || set.has('THE_HEALER')))
        return 'THE_FIRE_AND_THE_WATER';
    if (set.has('THE_SEEKER') && set.has('THE_WALL'))
        return 'THE_CHASER_AND_THE_WALL';
    if (set.has('THE_GUARDIAN') && set.has('THE_SEEKER'))
        return 'THE_GUARDIAN_AND_THE_SEEKER';
    if (set.has('THE_ANCHOR') && set.has('THE_FREE_SOUL'))
        return 'THE_ANCHOR_AND_THE_FREE_SOUL';
